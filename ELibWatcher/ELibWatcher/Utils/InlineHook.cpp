...
bool InlineHook::AddHook(LPVOID pTarget, InlineFunc beforeFunc, InlineFunc afterFunc)
{
	...
	memcpy(&ShellCode[0x4], &beforeFunc, 4);
	//保存原始机器码
	DWORD dwOldProtect = 0;
	BOOL bRet = VirtualProtectEx(hProcess, pTarget, 0x20, PAGE_EXECUTE_READWRITE, &dwOldProtect);
	if (!bRet)
	{
		return false;
	}
	...
	*(DWORD*)(jmpcode + 1) = m_dwVirtualAddr - (DWORD)pTarget - 5;
	...
	//还原内存属性
	VirtualProtectEx(hProcess, pTarget, 5, dwOldProtect, &dwOldProtect);
	return true;
}
